...
格式归一化组件：把 docx 的标题层级 / 粗体 / 下划线 / 列表 / 图片 / 表格
...
def extract(docx_path, out_md):
    try:
        ...
        print(f"[FAIL] 不是合法 docx：{e}", file=sys.stderr)
        return 1
    ...
    lines, img_n, tbl_n = [], 0, 0

    body = doc.find(f"{W}body")
    for el in body:
        tag = el.tag
        if tag == f"{W}tbl":
            # 表格 → Markdown 表格，保住行列结构
            rows = []
            for tr in el.findall(f"{W}tr"):
                cells = []
                for tc in tr.findall(f"{W}tc"):
                    cell_text = "".join(t.text or "" for t in tc.iter(f"{W}t"))
                    cell_text = cell_text.strip().replace("|", "\\|") or " "
    ...
    with open(out_md, "w", encoding="utf-8") as f:
        f.write(md)
    print(f"[OK] {os.path.basename(docx_path)} → {out_md}")
    print(f"  段落 {sum(1 for l in lines if l and not l.startswith(('#','-','![')))} · "
          f"标题 {sum(1 for l in lines if l.startswith('#'))} · "
          f"列表 {sum(1 for l in lines if l.startswith('- '))} · "
          f"图片 {img_n} · 表格 {tbl_n}")
    return 0


def main():
    ap = argparse.ArgumentParser(description="Word .docx → Markdown 提取器")
    ap.add_argument("docx", help="输入的 .docx 文件路径")
    ap.add_argument("-o", "--out", help="输出 md 路径（默认同名 .md）")
    args = ap.parse_args()
    if not os.path.isfile(args.docx):
        print(f"[FAIL] 文件不存在: {args.docx}", file=sys.stderr)
        sys.exit(1)
