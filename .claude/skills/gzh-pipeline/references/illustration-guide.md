...
| 卡片中的默认色 | 深蓝商务 | → 暖橙故事 | → 科技暗色 | → **极简清爽★** |
|---------------|---------|-----------|-----------|-----------------|
| `#0a1f3d`（暗底/主色） | 保持不变 | `#3D2E1E` | `#1A1A2E` | `#F5F7F6`（浅灰绿底） |
| `#315d93`（强调色） | 保持不变 | `#C4814A` | `#00D4FF` | `#7A9E8E` |
| `#f2f4f5`（浅背景） | 保持不变 | `#F5F0EB` | `rgba(255,255,255,0.06)` | `#FBFCFC` |
| `#e5ebef`（分割线/边框） | 保持不变 | `#E5D8CC` | `#2A2A3E` | `#EEEEEE` |
| `#5f6d78`（次要文字） | 保持不变 | `#9B8B7A` | `#6A7A8A` | `#999999` |

> **极简清爽特别规则**：不使用暗底卡片。所有卡片背景为白色或极浅灰，用细边框（1px solid #EEEEEE）代替深色背景。
> 序号用极浅灰底 + 强调色文字，不用实心圆形。
...
> 极简清爽主题的特别说明：该主题的所有卡片应使用更简洁的视觉语言——细边框代替深色背景，
> 白色背景为主，序号用透明底+强调色文字（不用实心圆形），间距更大，装饰元素更少。
> 以下类型 0 是专门为极简清爽主题设计的轻量文字卡。

### 类型 0：轻量文字卡（Light Text Card）

**适用场景**：极简清爽主题下的金句/关键概念展示、核心观点提炼、CTA 收尾

**设计原则**：无背景色块、无装饰元素、纯靠排版和间距传递信息

**模板：**

```html
<div class="illu-card" id="illu-N" style="padding: 24px 20px; font-family: 'Inter', 'Noto Sans SC', -apple-system, sans-serif; border-bottom: 1px solid #EEEEEE;">
  <button class="illu-download-btn" onclick="downloadCard('illu-N', '{{文件名}}')">⬇</button>
  <div style="font-size: 11px; color: #999999; letter-spacing: 3px; text-transform: uppercase; margin-bottom: 12px; font-weight: 400;">{{章节标签}}</div>
  <div style="font-size: 22px; font-weight: 300; color: #1A1A1A; line-height: 1.6; margin-bottom: 8px;">{{核心金句}}</div>
  <div style="font-size: 14px; color: #999999; font-weight: 300; line-height: 1.7;">{{补充说明（可选）}}</div>
</div>
```

**注意事项**：
- 无背景色、无边框卡片——直接用文字+底部细线
- 字体用 300 字重（Light），标题用 300 字重但字号 22px
- 标签用全大写 + 宽字距，营造杂志感
- 适用于金句卡、章节过渡卡、CTA 收尾卡
- 极简清爽主题优先选用此类型
...
| 内容特征 | 推荐卡片类型 | 适用主题 |
|---------|------------|---------|
| 单句金句/核心判断/CTA | **类型 0：轻量文字卡** | 极简清爽 ★ |
| 有步骤/流程/时序 | 类型 1：流程图 | 深蓝商务/暖橙故事/科技暗色 |
| 多个并列概念/功能点 | 类型 2：网格卡片 | 深蓝商务/暖橙故事/科技暗色 |
| A vs B / 新旧 / 有无对比 | 类型 3：暗底对比卡 | 深蓝商务/科技暗色 |
| 多维评分/权重/数据对比 | 类型 4：柱状图 | 深蓝商务/科技暗色 |
| 两种视角/投入产出/方案对比 | 类型 5：分栏布局 | 深蓝商务/暖橙故事 |

> **极简清爽主题优先使用类型 0**。其余类型使用时，去除深色背景、去除实心圆形序号、用细边框分隔。
